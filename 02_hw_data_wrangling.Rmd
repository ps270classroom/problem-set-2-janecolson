...
knitr::kable(incumbency_table,
             col.names = c("Incumbency Status", "Number of Candidates"))
```
There are 4532 candidates in the data set.
...
```{r}
filtered_data <- fb_ads %>%
  filter(office %in% c("US House", "US Senate"))

party_incumbent_promote <- filtered_data %>%
...
```
On average, democratic incumbents sponsored the most promoting ads.
...
us_house_data <- fb_ads |>
  filter(office == "US House")

plot_q3 <- ggplot(us_house_data, aes(x = impressions_millions)) +
  geom_histogram() +
  labs(title = "Facebook Ad Impressions (in millions) of US House Candidates",
...
The shape of the histogram decreases exponentially as x (Impressions (in millions)) increases. Most of the candidates did not have more than ten million ad impressions on Facebook.
...
library(dplyr)
library(ggplot2)
## Filter data for US House candidates
us_house_data <- fb_ads |>
  filter(office == "US House") |>
  arrange(desc(impressions_millions)) |>
  slice_head(n = 15) |>
  mutate(cand_name_last = factor(cand_name_last,
                                 levels = cand_name_last[order(impressions_millions)]))

## Barplot
fb_top_plot <- ggplot(us_house_data,
                      aes(x = impressions_millions,
                          y = cand_name_last)) +
  geom_bar(aes(fill = party), stat = "identity") +
  labs(title = "Top 15 US House Candidates by Facebook Ad Impressions",
       x = "Facebook Ad Impressions (millions)",
